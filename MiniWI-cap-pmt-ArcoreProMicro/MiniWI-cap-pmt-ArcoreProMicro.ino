...
//Requires the Arcore arduino core
...
DATE:                 2016-06-01
FILE SAVED AS:        MiniWI-cap-pmt-ArcoreProMicro.ino
...
void setup() {
  ...
  pinMode(16,INPUT_PULLUP); // panic pin 1/2
  pinMode(14,INPUT_PULLUP); // panic pin 2/2
  ...
  if (state == NOTE_OFF) {
    ...
    if (pressureSensor > ON_Thr) {
      ...
      if (millis() - breath_on_time > ON_Delay) {
        ...
        noteOn(MIDIchannel, fingeredNote, velocity);
    ...
    if (pressureSensor < ON_Thr) {
      ...
      noteOff(MIDIchannel, activeNote, velocity);
      breathLevel=0;
      state = NOTE_OFF;
    } else {
      ...
      // than the debounce delay, so take it as the actual current state
        if (fingeredNote != activeNote) {
          ...
          noteOff(MIDIchannel, activeNote, 0);
          activeNote=fingeredNote;
        }
      }
    }
...
void pitch_bend(){
  ...
  if (pitchBend != oldpb){// only send midi data if pitch bend has changed from previous value
    ...
    pitchBendChange(MIDIchannel, pitchBend);
    oldpb=pitchBend;
  }
}
...
void modulation(){
  ...
  if (modLevel != oldmod){  // only send midi data if modulation has changed from previous value
    ...
    controlChange(MIDIchannel, 1, modLevel);
    oldmod=modLevel;
  }
}
...
void portamento(){
  ...
  if (portLevel != oldport){  // only send midi data if level has changed from previous value
    ...
    controlChange(MIDIchannel, 5, portLevel);   
    oldport=portLevel;
  }
  if (PortK != oldportk){  // only send midi data if status has changed from previous value
    if (PortK){ 
      ...
      controlChange(MIDIchannel, 65, 127);
    }
    else {
      midiSend((0xB0 | MIDIchannel), 65, 0); // send portamento off  
      controlChange(MIDIchannel, 65, 0); 
    }
    oldportk=PortK;
  }
}
...
void breath(){
  ...
  controlChange(MIDIchannel, 2, breathCC);
}
//***********************************************************
